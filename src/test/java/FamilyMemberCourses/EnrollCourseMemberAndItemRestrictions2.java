...
//	@BeforeTest
	 ...
	 * Assert.assertEquals("Select Courses / Events",
	 * BT.getBreadcrumb2().getText());
	 * 
	 * WebDriverWait wait = new WebDriverWait(driver, 50);
	 * wait.until(ExpectedConditions.refreshed(ExpectedConditions.
	 * presenceOfElementLocated(By.id("courses"))));
	 * 
	 * WebElement MonthNames =
	 * driver.findElement(By.xpath("//div[@class='col-md-9']")); int monthCount =
	 * MonthNames.findElements(By.tagName("label")).size(); for (int i = 0; i <
	 * monthCount; i++) { String monthName =
	 * MonthNames.findElements(By.tagName("label")).get(i).getText(); if
	 * (monthName.equals(CourseStartMonth2)) {
	 * MonthNames.findElements(By.tagName("label")).get(i).click(); break; }
	 * 
	 * }
	 * 
	 * wait.until(ExpectedConditions.refreshed(ExpectedConditions.
	 ...
	 * 
	 * for (int j = 0; j < CourseCount; j++) { String courseName =
	 ...
	 * System.out.println(c.getPopUpErrorMessage().getText().trim()); Assert.
	 * assertEquals("Membership restrictions have limited enrollment into this course."
	 ...
	 * c.getPopupCancelButtonCourse().click(); Thread.sleep(1000);
	 * 
	 ...
	 * "Validating that the course cannnot be enrolled in when the enrollment window has closed"
	 * ) public void CourseEnrollmentWindowClosed() throws IOException,
	 * InterruptedException {
	 * 
	 ...
	 * } Thread.sleep(1000); try {
	 * System.out.println(c.getPopUpErrorMessage().getText().trim()); Assert.
	 * assertEquals("The online enrollment window for this course has closed.",
	 * c.getPopUpErrorMessage().getText().trim());
	 * Assert.assertFalse(c.getPopupSignupButtonCourse().isEnabled());
	 * 
	 * } catch (java.lang.AssertionError ae) {
	 ...
	 * "Validating that the course cannnot be enrolled due to Item Restrictions")
	 ...
	 * "Validating that the course cannnot be enrolled due to the enrollment window is not open yet"
	 * ) public void CourseEnrollmentNotOpened() throws IOException,
	 * InterruptedException {
	 * 
	 * reusableMethods.activeMemberLogin("feemember", "Testing1!");
	 * reusableWaits.waitForDashboardLoaded();
	 ...
	 * "Validating that the course cannnot be enrolled as the course time is passed"
	 * ) public void CourseEnrollmentEnded() throws IOException,
	 * InterruptedException {
	 * 
	 * reusableMethods.activeMemberLogin("feemember", "Testing1!");
	 * reusableWaits.waitForDashboardLoaded();
	 ...
	 * WebDriverWait wait = new WebDriverWait(driver, 50);
	 * wait.until(ExpectedConditions.refreshed(ExpectedConditions.
	 * presenceOfElementLocated(By.id("courses")))); String year =
	 * driver.findElement(By.xpath("//span[contains(@class, 'btn-white')]")).getText
	 ...
	 * monthCount; i++) { String monthName =
	 * MonthNames.findElements(By.tagName("label")).get(i).getText(); if
	 * (monthName.equals(CourseStartMonth1)) {
	 * MonthNames.findElements(By.tagName("label")).get(i).click(); break; }
	 * 
	 * }
	 * 
	 ...
	 * "Validating that the course cannnot be enrolled due to Membership Type Time Restrictions"
	 * ) public void CourseOutsidePermittedHours() throws IOException,
	 * InterruptedException {
	 * 
