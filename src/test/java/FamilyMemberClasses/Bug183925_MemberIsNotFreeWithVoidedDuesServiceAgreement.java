...
public class Bug183925_MemberIsNotFreeWithVoidedDuesServiceAgreement extends base {
	...
	private static String member = "Bhagya";
	private static String memberRate = "$5.00";
	private static JavascriptExecutor jse;

	public reusableWaits rw;
	public reusableMethods rm;

	public Bug183925_MemberIsNotFreeWithVoidedDuesServiceAgreement() {
...
//	@BeforeTest
	...
	@Test(priority = 1)
	public void verifyMemberIsNotFree() throws IOException, InterruptedException {
		try {
			...
			for (int j = 0; j < ClassCount; j++) {
...

				{
					...
					for (int k = 0; k < count; k++) {
						...
						if (getMemberRate.get(k).getText().contains(member))
							Assert.assertTrue(getMemberRate.get(k).getText().contains(memberRate));
